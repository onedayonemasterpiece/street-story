...
class FeedProjectionInstrumentedTest {
    ...
    @Test fun feedShowsTenThreadsCleanedBubblesAndFixedRecordingControls() {
        val context = InstrumentationRegistry.getInstrumentation().targetContext
        ...
        store.setStage(created.first().clientStoryId, StoryStage.RESEARCHING)
        store.setStage(target.clientStoryId, StoryStage.REVIEW)
        store.setDraftText(target.clientStoryId, "Первая строка\nВторая строка\nТретья строка\nЧетвёртая строка\nПятая строка")
        store.replaceFacts(target.clientStoryId, listOf(
            FactSnapshot("fact-supported", "Подтверждённый факт", .9, true, true, "[{\"url\":\"https://example.test/source\"}]"),
            FactSnapshot("fact-unsupported", "Неподтверждённый факт", .2, false, false, "[]"),
        ))
        store.replaceDestinations(target.clientStoryId, listOf(
        ...
        val voice1 = VoiceMessageWire().apply {
            sessionId = "voice-feed-clean-1"; kind = RecordingKind.INITIAL
            rawTranscript = "э-э, я я хочу про Дом Советов"; displayText = "Я хочу про Дом Советов"
        }
        val voice2 = VoiceMessageWire().apply {
            sessionId = "voice-feed-clean-2"; kind = RecordingKind.REFINEMENT
            rawTranscript = "а-а, ещё ещё про площадь"; displayText = "Ещё про площадь"
        }
        FeedProjectionStore(context).use { it.replaceVoiceMessages(target.clientStoryId, listOf(voice1, voice2)) }
        context.getSharedPreferences("street_story_ui", Context.MODE_PRIVATE).edit().putString("active_story_id", target.clientStoryId).commit()
        ...
        ActivityScenario.launch(MainActivity::class.java).use { scenario ->
            scenario.onActivity { activity ->
                ...
                assertTrue(hidden.all { story -> findByDescription(root, "story-thread-${story.clientStoryId}") == null })
                ...
                assertEquals(2, collectByPrefix(root, "voice-message-").count { it.contentDescription.toString().contains("voice-feed-clean") })
    ...
    private fun findByDescription(view: android.view.View, description: String): android.view.View? {
        ...
        if (view is android.view.ViewGroup) {
            for (index in 0 until view.childCount) findByDescription(view.getChildAt(index), description)?.let { return it }
        }
        return null
    }

    private fun collectByPrefix(view: android.view.View, prefix: String, out: MutableList<android.view.View> = mutableListOf()): List<android.view.View> {
        if (view.contentDescription?.toString()?.startsWith(prefix) == true) out += view
        if (view is android.view.ViewGroup) for (index in 0 until view.childCount) collectByPrefix(view.getChildAt(index), prefix, out)
        return out
    }

    private fun collectText(view: android.view.View, out: MutableList<String> = mutableListOf()): List<String> {
        if (view is android.widget.TextView) out += view.text?.toString().orEmpty()
        if (view is android.view.ViewGroup) for (index in 0 until view.childCount) collectText(view.getChildAt(index), out)
        return out
    }
