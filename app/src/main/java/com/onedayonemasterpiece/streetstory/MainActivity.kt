...
class MainActivity : Activity() {
    ...
    private fun selectStory(id: String?) {
        selectedStoryId = id
        uiPrefs.edit().apply {
            if (id == null) remove("story_id") else putString("story_id", id)
        }.apply()
    }
    ...
    private fun renderFeed(page: LinearLayout) {
        ...
        page.addView(
            label(
                "Фото, голос, проверенные факты — и публикация уже ждёт своего часа.",
                18,
                GRAPHITE_SOFT,
                bodyLight,
            ).apply {
        ...
        if (!config.configured) {
            ...
            setup.addView(
                label(
                    "Фото и голос сохранятся на телефоне. Для исследования и публикации один раз укажи HTTPS-адрес Street Story backend на DevCoveer.",
                    16,
                    MUTED,
                    bodyLight,
                ),
        ...
        page.addView(
            action(primaryText, ORANGE, WHITE, ORANGE, 0) {
                if (active != null) {
                    selectStory(active.storyId)
                    render()
                } else {
                    launchPhotoPicker()
                }
        ...
        if (stories.isEmpty()) {
            ...
            empty.addView(
                label(
                    "Сначала фотография надёжно останется на телефоне. Потом можно спокойно наговорить даже длинную заметку и закрыть приложение.",
                    17,
                    0xffcbd0cb.toInt(),
                    editorial,
                ),
    ...
    private fun feedCard(story: StorySnapshot): View {
        val card = surface(PAPER, 26).apply {
            setPadding(dp(12), dp(12), dp(14), dp(12))
            isClickable = true
            isFocusable = true
            setOnClickListener {
                selectStory(story.clientStoryId)
                render()
            }
        }
        ...
        copy.addView(
            micro(
                statusText(story.stage).uppercase(Locale.getDefault()),
                if (story.stage == StoryStage.NEEDS_REVIEW || story.stage == StoryStage.VISUAL_BLOCKED) ORANGE else MUTED,
            ),
        )
        copy.addView(
            label(story.placeName ?: "Новая городская история", 21, INK, display).apply {
                setPadding(0, dp(5), 0, dp(4))
                maxLines = 2
            },
        )
        val destinations = store.destinations(story.clientStoryId).filter { it.selected }.joinToString(" · ") {
            it.provider.uppercase(Locale.US)
        }
        val meta = listOf(
            dateText(story.createdAt),
            destinations.takeIf { it.isNotBlank() },
            scheduleText(story.scheduledFor),
    ...
    private fun renderStory(page: LinearLayout, story: StorySnapshot) {
        page.addView(
            micro("←  STREET STORY", MUTED).apply {
                isClickable = true
                setPadding(dp(3), dp(2), 0, dp(14))
                setOnClickListener { showFeed() }
            },
        )
        val active = store.activeVoiceSession()
        if (active != null && active.storyId == story.clientStoryId) {
            renderVoice(page, story, active)
            return
        }
        when (story.stage) {
            ...
            StoryStage.READY_TO_PUBLISH,
            StoryStage.NEEDS_REVIEW,
            -> renderReview(page, story)
    ...
    private fun renderVoice(page: LinearLayout, story: StorySnapshot, active: VoiceSessionSnapshot?) {
        ...
        val kind = active?.kind ?: if (
            store.latestVoiceSession(story.clientStoryId)?.kind == RecordingKind.INITIAL && story.stage != StoryStage.PHOTO_READY
        ) {
        ...
        page.addView(
            label(if (kind == RecordingKind.REFINEMENT) "Что уточнить?" else "Что здесь интересно?", 38, INK, display).apply {
        ...
        page.addView(
            label(
                "Говори как есть. Тишина автоматически пропускается, длинная запись складывается в durable M4A-чанки — закрывать экран и приложение можно.",
                17,
                GRAPHITE_SOFT,
                bodyLight,
            ).apply {
        ...
        if (active == null) {
            ...
            hero.addView(
                label(formatDuration(duration), 48, PAPER, display).apply {
            ...
            hero.addView(
                label(
                    "${active.chunkCount} сохранённых чанков · ${formatDuration(live?.autoSilenceSkippedMs ?: active.autoSilenceSkippedMs)} тишины пропущено",
                    15,
                    0xffcbd0cb.toInt(),
                    body,
                ),
            ...
            row.addView(
                action(if (paused) "Продолжить" else "Пауза", PAPER, INK, INK, 1) {
            ...
            row.addView(
                action("Готово", ORANGE, WHITE, ORANGE, 0) {
                    RecordingService.command(this, RecordingService.ACTION_FINISH)
                },
                LinearLayout.LayoutParams(0, dp(58), 1f).apply { setMargins(dp(8), 0, 0, 0) },
            )
            page.addView(row)
        }
    }
    ...
    private fun renderProcessing(page: LinearLayout, story: StorySnapshot) {
        ...
        hero.addView(
            label(statusText(story.stage), 36, PAPER, display).apply {
        ...
        hero.addView(
            label(
                if (config.configured) {
                    "Можно закрыть приложение. WorkManager продолжит отправку и сверку с сервером."
                } else {
                    "Сервер пока не настроен. Фото и голос остаются локально и никуда не пропадут."
                },
                17,
                0xffcbd0cb.toInt(),
                editorial,
        ...
        page.addView(
            action(if (config.configured) "Проверить сейчас" else "Настроить backend", PAPER, INK, INK, 1) {
                if (config.configured) SyncScheduler.enqueue(this) else showSettings()
            },
        )
    }
    ...
    private fun renderReview(page: LinearLayout, story: StorySnapshot) {
        ...
        page.addView(
            micro(
                statusText(story.stage).uppercase(Locale.getDefault()),
                if (story.stage == StoryStage.VISUAL_BLOCKED || story.stage == StoryStage.NEEDS_REVIEW) ORANGE else MUTED,
            ),
        )
        page.addView(
            label(story.placeName ?: "Что удалось узнать", 36, INK, display).apply {
        ...
        story.summary?.takeIf { it.isNotBlank() }?.let {
            page.addView(
                label(it, 17, GRAPHITE_SOFT, bodyLight).apply {
                    setLineSpacing(dp(1).toFloat(), 1.08f)
            ...
            facts.forEach { fact ->
                ...
                val check = CheckBox(this).apply {
                    ...
                    setOnCheckedChangeListener { _, checked ->
                        store.setFactSelected(story.clientStoryId, fact.factId, checked)
                    }
                }
                wrap.addView(check)
                ...
                wrap.addView(
                    label(
                        if (fact.evidenceSupported) {
                            "$sourceCount источн. · confidence ${"%.2f".format(Locale.US, fact.confidence)}"
                        } else {
                            "Нет внешнего evidence — в публикацию не включается"
                        },
                        13,
                        if (fact.evidenceSupported) MUTED else ORANGE,
                        body,
                    ).apply { setPadding(dp(34), dp(3), 0, 0) },
                )
        ...
        if (story.stage == StoryStage.VISUAL_BLOCKED) {
            page.addView(
                messageSurface(
                    "Imagegen runtime VibePublish сейчас недоступен. Исследование, факты, голос и исходное фото сохранены; новую story создавать не нужно.",
                ),
                margins(0, 0, 0, 12),
            )
        }
        ...
        if (
            latest?.captureState != CaptureState.RECORDING &&
            latest?.captureState != CaptureState.PAUSED &&
            story.stage != StoryStage.SCHEDULING
        ) {
            page.addView(
                action("Уточнить голосом", PAPER, INK, INK, 1) {
        ...
        when (story.stage) {
            StoryStage.REVIEW, StoryStage.VISUAL_BLOCKED, StoryStage.NEEDS_REVIEW -> page.addView(
                action(
                    if (story.stage == StoryStage.VISUAL_BLOCKED) "Повторить изображение" else "Готовить изображение",
                    ORANGE,
                    WHITE,
                    ORANGE,
                    0,
            ...
            StoryStage.VISUAL_PROCESSING -> page.addView(
                action("Проверить обработку", PAPER, INK, INK, 1) { SyncScheduler.enqueue(this) },
            )
    ...
    private fun renderPublishControls(page: LinearLayout, story: StorySnapshot) {
        ...
        val edit = EditText(this).apply {
            setText(story.draftText.orEmpty())
            setTextSize(17f)
            setTextColor(INK)
            typeface = body
            background = null
            ...
            addTextChangedListener(
                object : TextWatcher {
                    ...
                    override fun afterTextChanged(s: Editable?) {
                        store.setDraftText(story.clientStoryId, s?.toString().orEmpty())
                    }
                },
            )
        }
        copySurface.addView(edit)
        page.addView(copySurface, margins(0, 0, 0, 12))
        ...
        if (destinations.isEmpty()) {
            destSurface.addView(
                label(
                    "VibePublish пока не вернул доступных destination. Ничего не придумываем локально.",
                    16,
                    MUTED,
                    bodyLight,
                ).apply { setPadding(0, dp(9), 0, dp(8)) },
            )
        } else {
            destinations.forEach { destination ->
                ...
                destSurface.addView(
                    CheckBox(this).apply {
                        ...
                        buttonTintList = ColorStateList.valueOf(ORANGE)
                        setOnCheckedChangeListener { _, checked ->
                            store.setDestinationSelected(story.clientStoryId, destination.alias, checked)
                        }
                    },
                )
            }
        ...
        page.addView(
            action("В отложенные · через 1 час", ORANGE, WHITE, ORANGE, 0) {
                queuePublish(requireNotNull(store.story(story.clientStoryId)))
            },
        )
    }
    ...
    private fun renderFinal(page: LinearLayout, story: StorySnapshot) {
        ...
        page.addView(
            label(story.placeName ?: "Городская история", 38, INK, display).apply {
                setPadding(0, dp(9), 0, dp(7))
                letterSpacing = -0.025f
            },
        )
        ...
        page.addView(
            label(
                if (story.stage == StoryStage.PUBLISHED) {
                    "Публикация подтверждена сервером."
                } else {
                    "Provider-native публикация запланирована${scheduled?.let { " · $it" } ?: ""}."
                },
                17,
                GRAPHITE_SOFT,
                bodyLight,
        ...
        val destinations = store.destinations(story.clientStoryId).filter { it.selected }.joinToString(" · ") {
            "${it.label} / ${it.provider.uppercase(Locale.US)}"
        }
    ...
    private fun queueVisual(story: StorySnapshot) {
        ...
        if (store.pendingOperations(story.clientStoryId).any { it.kind == "visual" }) {
            SyncScheduler.enqueue(this)
            return
        }
        ...
        store.enqueueOperation(
            story.clientStoryId,
            "visual",
            newRequestKey("visual", "${story.clientStoryId}-${System.currentTimeMillis()}"),
            payload,
        )
    ...
    private fun queuePublish(story: StorySnapshot) {
        ...
        if (store.pendingOperations(story.clientStoryId).any { it.kind == "publish" }) {
            SyncScheduler.enqueue(this)
            return
        }
        ...
        store.enqueueOperation(
            story.clientStoryId,
            "publish",
            newRequestKey("publish", "${story.clientStoryId}-${System.currentTimeMillis()}"),
            payload,
        )
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        Thread {
            runCatching {
                ...
                runOnUiThread {
                    Toast.makeText(this, "Не удалось сохранить фото: ${exc.message}", Toast.LENGTH_LONG).show()
                }
            }
        }.start()
    }
    ...
    private fun showSettings() {
        ...
        val token = EditText(this).apply {
            hint = if (config.deviceToken.isNullOrBlank()) {
                ...
                "Device token сохранён · оставь пустым, чтобы не менять"
            }
    ...
    private fun imageFrame(path: String, widthDp: Int, heightDp: Int): FrameLayout {
        val frame = FrameLayout(this).apply {
            background = rounded(PAPER, 26)
            clipToOutline = true
        }
        val image = ImageView(this).apply {
            scaleType = ImageView.ScaleType.CENTER_CROP
            decodeSampled(path, if (widthDp > 0) widthDp * 3 else 1200, heightDp * 3)?.let { setImageBitmap(it) }
        }
        frame.addView(image, FrameLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT))
        frame.layoutParams = if (widthDp > 0) {
            LinearLayout.LayoutParams(dp(widthDp), dp(heightDp))
        } else {
            LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, dp(heightDp))
        }
        return frame
    }
    ...
    private fun messageSurface(text: String): LinearLayout = surface(0xffffeee8.toInt(), 22).apply {
        setPadding(dp(16), dp(14), dp(16), dp(14))
        addView(label(text, 15, INK, body).apply { setLineSpacing(dp(1).toFloat(), 1.08f) })
    }
    ...
    private fun captureStatus(activity: String): String = when (activity) {
        ...
        CaptureActivity.AUTO_SILENCE -> "ТИШИНА · НЕ ЗАПИСЫВАЕТСЯ"
        CaptureActivity.FALLBACK_CONTINUOUS -> "ЗАПИСЫВАЮ ВСЁ · VAD FAIL-OPEN"
        CaptureActivity.MANUAL_PAUSE -> "ПАУЗА"
        else -> "ЗАПИСЬ"
    }

    private fun dateText(epoch: Long): String = DateTimeFormatter.ofPattern("d MMM", Locale.forLanguageTag("ru-RU"))
    ...
    private fun scheduleText(value: String?): String? = value?.let { raw ->
        runCatching {
            DateTimeFormatter.ofPattern("HH:mm", Locale.US)
                .format(OffsetDateTime.parse(raw).atZoneSameInstant(ZoneId.systemDefault()))
        }.getOrElse { raw }
    }
    ...
    private fun surface(color: Int, radius: Int) = LinearLayout(this).apply {
        orientation = LinearLayout.VERTICAL
    ...
    private fun buttonBackground(fill: Int, stroke: Int, strokeDp: Int, radius: Int) = RippleDrawable(
    ...
    private fun action(
        ...
        strokeDp: Int,
        action: () -> Unit,
    ) = label(textValue, 17, textColor, display).apply {
        gravity = Gravity.CENTER
        minHeight = dp(58)
        setPadding(dp(16), 0, dp(16), 0)
        background = buttonBackground(fill, stroke, strokeDp, 22)
        isClickable = true
        isFocusable = true
